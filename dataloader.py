...
class Mlens(BasicDataset):
    def __init__(self,path="../data/ml-1m"):
        ...
        print(f"ml-1m Sparsity : {(len(self.trainUser) + len(self.testUser))/self.n_users/self.m_items}")
...
class Ml100K(BasicDataset):
    def __init__(self,path="../data/ml-100k"):
        ...
        print(f"ml-100k Sparsity : {(len(self.trainUser) + len(self.testUser))/self.n_users/self.m_items}")
        self.UserItemNet  = csr_matrix((np.ones(len(self.trainUser)), (self.trainUser, self.trainItem) ), shape=(self.n_users,self.m_items)) 
        self._allPos = self.getUserPosItems(list(range(self.n_users)))
        self.allNeg = []
        allItems    = set(range(self.m_items))
...
class dblp(BasicDataset):
    def __init__(self,path="../data/dblp"):
        ...
        print(f"dblp Sparsity : {(len(self.trainUser) + len(self.testUser))/self.n_users/self.m_items}")
        self.UserItemNet  = csr_matrix((np.ones(len(self.trainUser)), (self.trainUser, self.trainItem) ), shape=(self.n_users,self.m_items)) 
        self._allPos = self.getUserPosItems(list(range(self.n_users)))
        self.allNeg = []
        allItems    = set(range(self.m_items))
